...
def find_vaga_by_id(vaga_id: int):
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            if vaga:
                return {
                    "id": vaga[0], "titulo_vaga": vaga[1], "criterios_de_analise": vaga[2], 
                    "top_x_candidatos": vaga[3], "criado_em": vaga[4], "finalizada_em": vaga[5]
                }
    return None

def find_all_vagas():
    with get_db_connection() as conn:
        sql = "SELECT id, titulo_vaga, criado_em, finalizada_em FROM vagas WHERE finalizada_em IS NULL ORDER BY criado_em DESC;"
        df = pd.read_sql_query(sql, conn)
        return df.to_dict(orient='records')

def create_new_vaga(vaga_data: dict):
    sql = """
        INSERT INTO vagas (titulo_vaga, criterios_de_analise, top_x_candidatos)
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (
                ...
                talento_data['sobre_mim'], json.dumps(talento_data['experiencia_profissional']),
...
def find_all_talentos():
    with get_db_connection() as conn:
        sql = "SELECT id, nome, email, formacao, vaga_id, criado_em FROM talentos ORDER BY criado_em DESC;"
        df = pd.read_sql_query(sql, conn)
        return df.to_dict(orient='records')
...
def save_ranking(vaga_id: int, ranking_data: list):
    ...
    insert_sql = """
        INSERT INTO top_aplicantes (vaga_id, talento_id, score_final, scores_por_criterio)
        VALUES (%s, %s, %s, %s);
    """
    with get_db_connection() as conn:
        with conn.cursor() as cur:
