...
def login_for_access_token(auth_data: AuthRequest):
    ...
    if not user or not auth_service.verify_password(auth_data.password, user['hashed_password']):
        raise HTTPException(
            ...
            detail="Email ou senha incorretos", # Mensagem mais direta para falha de login
            headers={"WWW-Authenticate": "Bearer"},
        )
    
    # --- ALTERAÇÃO AQUI ---
    # Verifica se o usuário está ativo. Se não estiver, lança um erro 403 Forbidden.
